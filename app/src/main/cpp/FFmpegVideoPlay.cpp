...
int FFmpegVideoPlay::playVideo() {
    ...
    LOGD("计算解码后的rgb %d\n", bufferSize);
    auto *outbuffer = (uint8_t *) av_malloc(bufferSize * sizeof(uint8_t));

    av_image_fill_arrays(rgbFrame->data, rgbFrame->linesize, outbuffer, AV_PIX_FMT_RGBA, width,
                         height, 1);
    //   转换器
    SwsContext *swsContext = sws_getContext(width, height, mAvCodecContext->pix_fmt,
...
//        读出来的数据是什么数据 视频   音频数据不管
        if (avPacket->stream_index == mVideoIndex) {
            ret = avcodec_send_packet(mAvCodecContext, avPacket);
            if (ret < 0 && ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                LOGD("解码出错");
                return -1;
            }

            ret = avcodec_receive_frame(mAvCodecContext, avFrame);
...
//            未压缩的数据
            sws_scale(swsContext, avFrame->data,
                      avFrame->linesize, 0,
                      mAvCodecContext->height, rgbFrame->data,
                      rgbFrame->linesize);
